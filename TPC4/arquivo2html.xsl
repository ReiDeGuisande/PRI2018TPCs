    ...
    <xsl:template match="obra" mode="obra">
        <xsl:result-document href ="website/html/obra{@id}.html">
            ...
            <body>
                ...
                <xsl:if test="arranjo">
                    <p><b>Arrajo de: </b> <xsl:value-of select="arranjo"/></p>
                </xsl:if>
                ...
                <p><address>[<a href="http://localhost:7777/obras">Voltar à página principal</a>]</address></p>
            </body>
        </xsl:result-document>
    </xsl:template>
    
<!-- Template para gerar a página do indice (???)-->
        ...
        <li>
            ...
            <ol>
                <xsl:for-each select="/arquivo-musical/obra[tipo=$t]">
                    ...
                    <li>
                        <a href="http://localhost:7777/obra?id={@id}">
                            <xsl:value-of select="titulo"/> <!-- estamos na obra -->
                        </a>
                    </li>
                </xsl:for-each>
            </ol>
